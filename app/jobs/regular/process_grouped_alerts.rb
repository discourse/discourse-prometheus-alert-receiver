...
module Jobs
  class ProcessGroupedAlerts < ::Jobs::Base
    ...
    def update_open_alerts(receiver, parsed_alerts, external_url:)
      parsed_alerts
        .each { |a| a[:topic_id] = receiver[:topic_map][a[:alertname]] }
        .reject! { |a| a[:topic_id].nil? }

      updated_topic_ids =
        AlertReceiverAlert.update_alerts(parsed_alerts, mark_stale_external_url: external_url)

      return if updated_topic_ids.blank?

      Topic
        .where(id: updated_topic_ids)
        .each do |topic|
