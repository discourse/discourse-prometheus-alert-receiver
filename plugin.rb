...
after_initialize do
  ...
  add_class_method(:topic, :alerts_category_ids_cache) do
    ...
    @alerts_category_ids_cache.defer_get_set("alert_category_ids") { alerts_category_ids }
  end
  ...
  add_to_serializer(:site, :firing_alerts_count) { Topic.firing_alerts.count }

  add_to_serializer(:site, :open_alerts_count) { Topic.open_alerts_count }

  add_to_serializer(:site, :include_open_alerts_count?) { scope.user&.include_alert_counts? }

  add_to_serializer(:site, :include_firing_alerts_count?) { scope.user&.include_alert_counts? }
  ...
  TopicQuery.add_custom_filter(
    ...
    if Topic.alerts_category_ids_cache.include?(options[:category_id]) &&
         options[:status] == "firing"
